...
<svelte:head>
    <title>My Portfoliio | {project.name}</title>
</svelte:head>
...
<h1 class="text-4xl font-semibold m-5">{project.name}</h1>

<div class="mb-5 flex justify-between">
    <div>
        {#if project.tags}
            {#each project.tags as tag}
                <span class="badge badge-primary mr-2 hover:bg-primary-focus cursor-pointer">
                    {tag}
                </span>
            {/each}
        {/if}
